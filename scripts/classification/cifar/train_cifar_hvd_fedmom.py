...
from gluoncv.trainer.fedmom_trainer import FedMomTrainer
...
def parse_args():
    ...
    parser.add_argument('--local-optimizer', type=str, default='sgd',
                        help='optimizer')
    parser.add_argument('--global-optimizer', type=str, default='sgd',
                        help='optimizer')
...
def main():
    ...
    logging.basicConfig(level=logging.INFO,
                    filename="train_cifar{}_fedmom_{}_{}_{}_{}_{}.log".format(classes, opt.model, opt.local_optimizer, opt.global_optimizer, opt.batch_size, opt.lr),
                    filemode='a')
    ...
    def train(epochs, ctx):
        ...
        if classes == 100:
            ...
            train_dataset = gluon.data.vision.CIFAR10(train=True).transform_first(transform_train)
            val_data = gluon.data.DataLoader(
        ...
        trainer = FedMomTrainer(net.collect_params(),  
                                opt.local_optimizer, local_optimizer_params, 
                                global_optimizer=opt.global_optimizer, 
                                global_optimizer_params=global_optimizer_params, 
        ...
        for epoch in range(epochs):
            ...
            # finish the remaining global optmization
            if trainer._local_sgd_counter > 0:
                trainer.global_step()
