...
class FedMomTrainer(mx.gluon.Trainer):
    ...
    def step(self, batch_size, ignore_stale_grad=False):
        ...
        for i, param in enumerate(self._params):
            if param.grad_req != 'null':
                self._grads_cache[i][:] += (param.list_grad()[0] * rescale_grad)
        ...
        if self._params_cache == []:
            ...
            for i, param in enumerate(self._params):
                if param.grad_req != 'null':
                    ...
                    self._grads_cache.append(zeros_like(param.list_data()[0]))
                else:
                    self._params_cache.append([])
                    self._grads_cache.append([])
    ...
    def _global_update(self):
        ...
        # set wd
        self._global_optimizer.wd = self._optimizer.wd * self._local_sgd_counter
